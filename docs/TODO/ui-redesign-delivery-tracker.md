...
| [x] | UI-05 | Ações do Command Center | Conectar start/stop por app, bulk stop se implementado via APIs existentes, toggles de auto-launch, auto-stop e `DO NOT stop apps when iRacing closes` | `CommandCenterScreen`, `api`, hooks | Mocks de API em testes; fluxo manual com app simples | 2026-05-02 |
| [x] | UI-06 | Apps administrativo | Reorganizar `AppsScreen` como biblioteca administrativa, mantendo add/edit/delete/start/stop e modal atual | `AppsScreen`, `AppCommandRow`, `AppFormModal` | Testes existentes de apps continuam passando | 2026-05-02 |
| [x] | UI-07 | Modal de app por seções | Refatorar add/edit para Basic, Launch, Recovery e Advanced com colapsáveis, validação e acessibilidade | `AppFormModal`, inputs/layout | Teste add/edit; teclado; erro com `role=alert` | 2026-05-02 |
...
| 2026-05-02 | Ações do Command Center usam APIs existentes | UI-05 conecta start/stop por app e bulk stop via `forceLaunchApp`/`forceKillApp`; toggles usam `updateApp` para `enabled`/`stop_with_iracing` e `setAutoStop` para o controle global `DO NOT stop apps when iRacing closes`. Bulk start foi removido do header por baixa clareza operacional. `Restart` não é exibido porque não há comportamento dedicado. |
| 2026-05-02 | Apps usa a mesma row operacional do Command Center | UI-06 reutiliza `AppCommandRow` em `AppsScreen` para alinhar leitura e estados; nessa tela a row recebe ações administrativas explícitas de editar/excluir e mantém start/stop manual. |
| 2026-05-02 | AppFormModal refatorado com seções Basic/Launch/Recovery/Advanced | Recovery e Advanced são colapsáveis (fechados por padrão); Basic e Launch usam `SectionDivider` estático (sempre abertos). Submit sempre habilitado; guard em `handleSubmit` marca campos obrigatórios como touched e exibe erros com `aria-invalid` + `role=alert`. `TextInput` ganhou props `aria-invalid` e `onBlur`; `FormField` ganhou prop `error` com `role=alert`. i18n renomeia Startup→Launch e Crash→Recovery, adiciona mensagens de validação. |
