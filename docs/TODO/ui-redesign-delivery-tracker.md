...
| [x] | UI-05 | Ações do Command Center | Conectar start/stop por app, bulk start/stop se implementado via APIs existentes, toggles de auto-launch, auto-stop e `DO NOT stop apps when iRacing closes` | `CommandCenterScreen`, `api`, hooks | Mocks de API em testes; fluxo manual com app simples | 2026-05-02 |
...
| 2026-05-02 | Ações do Command Center usam APIs existentes | UI-05 conecta start/stop por app e bulk start/stop via `forceLaunchApp`/`forceKillApp`; toggles usam `updateApp` para `enabled`/`stop_with_iracing` e `setAutoStop` para o controle global `DO NOT stop apps when iRacing closes`. `Restart` não é exibido porque não há comportamento dedicado. |
