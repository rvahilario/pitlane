...
| [x] | UI-02 | Novo modelo de navegação | Atualizar o tipo de tabs, i18n de navegação e sidebar para `command`, `apps`, `profiles`, `automation`, `logs`, `settings`, sem renderizar telas futuras como funcionais | `Sidebar`, `App.tsx`, i18n | Teste de tabs; confirmar que `Integrations` não aparece | 2026-05-01 |
...
| 2026-05-01 | Navegação V0 atualizada sem `Integrations` | Sidebar usa `command`, `apps`, `profiles`, `automation`, `logs`, `settings`; tabs ainda sem tela final renderizam placeholder sem ações falsas. |
