...
export function Sidebar({ active, onChange }: SidebarProps) {
    ...
    return (
        <aside className="flex w-shell-sidebar shrink-0 flex-col border-r border-border bg-base p-4">
            <nav className="flex flex-col gap-2">
                {NAV_ITEMS.map(({ id, labelKey, icon: Icon }) => (
                    <button
                        ...
                        onClick={() => onChange(id)}
                        className={cn(
                            'flex min-h-14 w-full items-center gap-4 rounded-lg border px-4 text-left text-body-ui font-medium transition-colors',
                            active === id
            ...
            <div className="mt-auto space-y-4">
                <div className="rounded-lg border border-border bg-surface px-4 py-4">
                    <div className="flex items-center gap-3">
                        ...
                        <span className="text-xs uppercase text-text-muted">Pitlane service</span>
                    </div>
                    <p className="mt-1 text-sm font-semibold text-success">Connected</p>
